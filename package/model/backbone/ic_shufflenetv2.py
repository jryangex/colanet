...
class ShuffleV2Block(nn.Module):
    ...
    def depthwise_conv(i, o, kernel_size, stride=1, padding=0, bias=False):
        global pattern, pattern_index
        #return ICConv2d( pattern[pattern_index], inplanes = i , planes = o, kernel_size=kernel_size,stride=stride,bias=bias)
...
class Ic_ShuffleNetV2(nn.Module):
    def __init__(self,
                 model_size='1.5x',
                 out_stages=(2, 3, 4),
                 ...
                 activation='ReLU',
                 pretrain=True):
        super(Ic_ShuffleNetV2, self).__init__()
        ...
        self.conv1 = nn.Sequential(
            ICConv2d(pattern[pattern_index], inplanes = output_channels , planes = output_channels, kernel_size=self.kernal_size,bias=False),
            nn.BatchNorm2d(output_channels),
            act_layers(activation),
        )
...
if __name__ == "__main__":
    model = Ic_ShuffleNetV2(model_size='1.0x', )
    print(model)
